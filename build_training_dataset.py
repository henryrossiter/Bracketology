...
for game in range(len(tournament_game_results_df.index)):
	...
	w_team_seed = seeds_df.loc[seeds_df['Season'] == year and seeds_df['TeamID'] == w_team_id].Seed[1:] #string
	l_team_seed = seeds_df.loc[seeds_df['Season'] == year and seeds_df['TeamID'] == l_team_id].Seed[1:] #string
	seed_diff = int(l_team_seed) - int(w_team_seed)
	w_team_in_tourney_last_year = (seeds['Season'] == year and seeds['TeamID'] == w_team_id)
	l_team_in_tourney_last_year = (seeds['Season'] == year and seeds['TeamID'] == l_team_id)
